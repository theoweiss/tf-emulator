...
public class BrickletAccelerometerTest {

  Vertx vertx;
  IPConnection ipcon;
  BrickletAccelerometer device;
  ...
  public void before(TestContext context) {
    ...
    JsonObject emuconfig = new JsonObject().put("devices", new JsonArray().add(
        new JsonObject().put("type", "BrickletAccelerometer").put("uid", uid).put("enabled", true)));
    ...
    device = new BrickletAccelerometer(uid, ipcon);
  }
  ...
  public void testgetTemperature(TestContext context) {
    try {
      short value = device.getTemperature();
  ...
  public void testgetAcceleration(TestContext context) {
    try {
      BrickletAccelerometer.Acceleration value = device.getAcceleration();
  ...
  @Test
  public void testled(TestContext context) {
    try {
      device.setResponseExpectedAll(true);
      device.ledOn(); //enable
      //assert enabled
      short value = device.isLEDOn();
      context.assertEquals(value, 1);

      device.ledOff(); //disable
      // assert disabled
      value = device.isLEDOn();
