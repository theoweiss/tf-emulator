...
public class BrickletRotaryEncoderTest {

  Vertx vertx;
  IPConnection ipcon;
  BrickletRotaryEncoder device;
  ...
  public void before(TestContext context) {
    ...
    JsonObject emuconfig = new JsonObject().put("devices", new JsonArray().add(
        new JsonObject().put("type", "BrickletRotaryEncoder").put("uid", uid).put("enabled", true)));
    ...
    device = new BrickletRotaryEncoder(uid, ipcon);
  }
  ...
  @Test
  public void test1getCount(TestContext context) {
    try {
      int value = device.getCount(true);
  ...
  @Test
  public void test2getCount(TestContext context) {
    try {
      int value = device.getCount(false);
      context.assertEquals(Utils.getRandomInt(), value);
