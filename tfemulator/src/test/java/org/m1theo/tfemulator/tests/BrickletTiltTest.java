...
public class BrickletTiltTest {
  ...
  public void before(TestContext context) {
    ...
    JsonObject emuconfig = new JsonObject().put("devices", new JsonArray().add(
        new JsonObject().put("type", "BrickletTilt").put("uid", uid).put("enabled", true)));
    ...
    device = new BrickletTilt(uid, ipcon);
  }
  ...
  @Test
  public void testgetTiltState(TestContext context) {
    try {
      short value = device.getTiltState();
  ...
  @Test
  public void testTiltStateCallback(TestContext context) {
    try {
      device.setResponseExpectedAll(true);
      device.enableTiltStateCallback(); //enable
      //assert enabled
      boolean value = device.isTiltStateCallbackEnabled();
context.assertTrue(value);

      device.disableTiltStateCallback(); //disable
      // assert disabled
      value = device.isTiltStateCallbackEnabled();
