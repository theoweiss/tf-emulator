...
public class BrickletRS232Test {

  Vertx vertx;
  IPConnection ipcon;
  BrickletRS232 device;
  ...
  public void before(TestContext context) {
    ...
    JsonObject emuconfig = new JsonObject().put("devices", new JsonArray().add(
        new JsonObject().put("type", "BrickletRS232").put("uid", uid).put("enabled", true)));
    ...
    device = new BrickletRS232(uid, ipcon);
  }
  ...
  @Test
  public void testReadCallback(TestContext context) {
    try {
      device.setResponseExpectedAll(true);
      device.enableReadCallback(); //enable
      //assert enabled
      boolean value = device.isReadCallbackEnabled();
context.assertTrue(value);

      device.disableReadCallback(); //disable
      // assert disabled
      value = device.isReadCallbackEnabled();
