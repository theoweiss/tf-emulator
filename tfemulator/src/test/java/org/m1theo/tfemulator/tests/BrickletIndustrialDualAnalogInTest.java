...
public class BrickletIndustrialDualAnalogInTest {

  Vertx vertx;
  IPConnection ipcon;
  BrickletIndustrialDualAnalogIn device;
  ...
  public void before(TestContext context) {
    ...
    JsonObject emuconfig = new JsonObject().put("devices", new JsonArray().add(
        new JsonObject().put("type", "BrickletIndustrialDualAnalogIn").put("uid", uid).put("enabled", true)));
    ...
    device = new BrickletIndustrialDualAnalogIn(uid, ipcon);
  }
  ...
  @Test
  public void testgetADCValues(TestContext context) {
    try {
      int[] value = device.getADCValues();
            for (int i = 0; i < value.length; i++) {
        context.assertEquals(Utils.getRandomInt(), value[i]);
      }
  ...
  @Test
  public void testgetVoltage(TestContext context) {
    try {
      int value = device.getVoltage((short) 1);
