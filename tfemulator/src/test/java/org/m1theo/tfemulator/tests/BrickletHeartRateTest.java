...
public class BrickletHeartRateTest {

  Vertx vertx;
  IPConnection ipcon;
  BrickletHeartRate device;
  ...
  public void before(TestContext context) {
    ...
    JsonObject emuconfig = new JsonObject().put("devices", new JsonArray().add(
        new JsonObject().put("type", "BrickletHeartRate").put("uid", uid).put("enabled", true)));
    ...
    device = new BrickletHeartRate(uid, ipcon);
  }
  ...
  @Test
  public void testgetHeartRate(TestContext context) {
    try {
  ...
  @Test
  public void testBeatStateChangedCallback(TestContext context) {
    try {
      device.setResponseExpectedAll(true);
      device.enableBeatStateChangedCallback(); //enable
      //assert enabled
      boolean value = device.isBeatStateChangedCallbackEnabled();
context.assertTrue(value);

      device.disableBeatStateChangedCallback(); //disable
      // assert disabled
      value = device.isBeatStateChangedCallbackEnabled();
