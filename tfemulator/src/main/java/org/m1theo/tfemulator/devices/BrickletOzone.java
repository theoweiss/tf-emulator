...
public final static String DEVICE_DISPLAY_NAME = "Ozone Bricklet";
  ...
  private Buffer ozoneConcentrationCallbackThreshold = Utils.getThresholdDefault();
  ...
  private short ozoneConcentration = 100;
  private short ozoneConcentration_max = 1000;
  private short ozoneConcentration_min = 0;
  private short ozoneConcentration_step = 1;
  private long ozoneConcentration_generator_period = 100;
  private Step ozoneConcentration_direction = Step.UP;
  private long ozoneConcentrationCallbackPeriod;
  private long ozoneConcentration_callback_id;
  private short ozoneConcentration_last_value_called_back;
  ...
  public void start() throws Exception {
    ...
    startOzoneConcentrationGenerator();
  }
  ...
  private void startOzoneConcentrationGenerator() {
    if (ozoneConcentration_step == 0) {
      return;
    }
    vertx.setPeriodic(ozoneConcentration_generator_period, id -> {
      if (ozoneConcentration_direction == Step.UP) {
        if (ozoneConcentration >= ozoneConcentration_max) {
          ozoneConcentration_direction = Step.DOWN;
          this.ozoneConcentration = (short) (ozoneConcentration - ozoneConcentration_step);
        } else {
          this.ozoneConcentration = (short) (ozoneConcentration + ozoneConcentration_step);
        }
      } else {
        if (ozoneConcentration <= ozoneConcentration_min) {
          ozoneConcentration_direction = Step.UP;
          this.ozoneConcentration = (short) (ozoneConcentration + ozoneConcentration_step);
        } else {
          this.ozoneConcentration = (short) (ozoneConcentration - ozoneConcentration_step);
        }
...
//fixme start_generator callback without sensor ozoneConcentrationReached
//fixme start_generator callback without sensor analogValueReached

    private void stopOzoneConcentrationCallback() {
        vertx.cancelTimer(ozoneConcentration_callback_id);
  }
...
//fixme stop_generator callback without sensor ozoneConcentrationReached
  ...
  private void startOzoneConcentrationCallback() {
    ...
    ozoneConcentration_callback_id = vertx.setPeriodic(ozoneConcentrationCallbackPeriod, id -> {
      if (ozoneConcentration != ozoneConcentration_last_value_called_back) {
        ...
        Set<Object> handlerids = vertx.sharedData().getLocalMap(Brickd.HANDLERIDMAP).keySet();
        if (handlerids != null) {
          logger.debug("ozoneConcentration sending callback value");
          for (Object handlerid : handlerids) {
            vertx.eventBus().publish((String) handlerid, getOzoneConcentration4Callback());
          }
        } else {
          logger.info("no handlerids found in ozoneConcentration callback");
        }
      } else {
        logger.debug("ozoneConcentration value already called back");
      }
    });
  }

//fixme stop_generator callback without sensor ozoneConcentration
...
//fixme getter callback without sensor ozoneConcentrationReached
//fixme getter callback without sensor analogValueReached

  private Buffer getOzoneConcentration4Callback() {
      byte options = (byte) 0;
      return getOzoneConcentrationBuffer(CALLBACK_OZONE_CONCENTRATION, options);
  }
        //fixme getter callback without sensor ozoneConcentration

  private Buffer setOzoneConcentrationCallbackPeriod(Packet packet) {
    ozoneConcentrationCallbackPeriod =  Utils.unsignedInt(packet.getPayload());
    if (ozoneConcentrationCallbackPeriod == 0) {
      stopOzoneConcentrationCallback();
    } else {
      startOzoneConcentrationCallback();
    }
    if (packet.getResponseExpected()) {
  ...
  private Buffer getOzoneConcentrationBuffer(byte functionId, byte options) {
    byte length = (byte) 8 + 2;
    byte flags = (byte) 0;
    Buffer header = Utils.createHeader(uidBytes, length, functionId, options, flags);
    Buffer buffer = Buffer.buffer();
    buffer.appendBuffer(header);
    buffer.appendBytes(Utils.getUInt16(ozoneConcentration));

    return buffer;
  }
        
  private Buffer getOzoneConcentrationBuffer(Packet packet) {
      byte options = packet.getOptions();
      return getOzoneConcentrationBuffer(FUNCTION_GET_OZONE_CONCENTRATION, options);
  }

  private Buffer getOzoneConcentration(Packet packet) {
    logger.debug("function getOzoneConcentration");
    if (packet.getResponseExpected()) {
      return getOzoneConcentrationBuffer(packet);
    }
    return null;
  }
