...
public final static String DEVICE_DISPLAY_NAME = "Color Bricklet";
  ...
  private Buffer light = getLightDefault();
...
//fixme getter callback without sensor colorReached
  ...
  private Buffer getColorCallbackPeriod(Packet packet) {
    ...
    if (packet.getResponseExpected()) {
      ...
      Buffer buffer = Buffer.buffer();
      buffer.appendBuffer(header);
    buffer.appendBytes(Utils.getUInt32(colorCallbackPeriod));
  ...
  private Buffer getColorTemperatureCallbackPeriod(Packet packet) {
    ...
    if (packet.getResponseExpected()) {
      ...
      Buffer buffer = Buffer.buffer();
      buffer.appendBuffer(header);
    buffer.appendBytes(Utils.getUInt32(colorTemperatureCallbackPeriod));
