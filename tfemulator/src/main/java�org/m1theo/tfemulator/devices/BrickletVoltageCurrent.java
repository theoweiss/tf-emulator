...
//fixme getter callback without sensor currentReached
  ...
  private Buffer getCurrentCallbackPeriod(Packet packet) {
    ...
    if (packet.getResponseExpected()) {
      ...
      Buffer buffer = Buffer.buffer();
      buffer.appendBuffer(header);
    buffer.appendBytes(Utils.getUInt32(currentCallbackPeriod));
  ...
  private Buffer getPowerCallbackPeriod(Packet packet) {
    ...
    if (packet.getResponseExpected()) {
      ...
      Buffer buffer = Buffer.buffer();
      buffer.appendBuffer(header);
    buffer.appendBytes(Utils.getUInt32(powerCallbackPeriod));
  ...
  private Buffer getVoltageCallbackPeriod(Packet packet) {
    ...
    if (packet.getResponseExpected()) {
      ...
      Buffer buffer = Buffer.buffer();
      buffer.appendBuffer(header);
    buffer.appendBytes(Utils.getUInt32(voltageCallbackPeriod));
