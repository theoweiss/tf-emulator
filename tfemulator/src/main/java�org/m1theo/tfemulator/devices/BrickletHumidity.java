 ...
 */
public class BrickletHumidity extends AbstractVerticle {
long uidBytes;
...
public final static String DEVICE_DISPLAY_NAME = "Humidity Bricklet";
  ...
  private Buffer humidityCallbackThreshold = getHumidityCallbackThresholdDefault();
        
  private short humidity = 100;
  private short humidity_max = 1000;
  private short humidity_min = 0;
  private short humidity_step = 1;
  private long humidity_generator_period = 100;
  private Step humidity_direction = Step.UP;
  private long humidityCallbackPeriod;
  private long humidity_callback_id;
  private short humidity_last_value_called_back;
  ...
  public void start() throws Exception {
    ...
    logger.info("Verticle started: " + BrickletHumidity.class);
  ...
  private Buffer callFunction(Packet packet) {
    ...
    else if (functionId == FUNCTION_GET_HUMIDITY) {
      buffer = getHumidity(packet);
    }
    else if (functionId == FUNCTION_GET_ANALOG_VALUE) {
      buffer = getAnalogValue(packet);
    }
    else if (functionId == FUNCTION_SET_HUMIDITY_CALLBACK_PERIOD) {
      buffer = setHumidityCallbackPeriod(packet);
    }
    else if (functionId == FUNCTION_GET_HUMIDITY_CALLBACK_PERIOD) {
      buffer = getHumidityCallbackPeriod(packet);
    }
    ...
    else if (functionId == FUNCTION_SET_HUMIDITY_CALLBACK_THRESHOLD) {
      buffer = setHumidityCallbackThreshold(packet);
    }
    else if (functionId == FUNCTION_GET_HUMIDITY_CALLBACK_THRESHOLD) {
      buffer = getHumidityCallbackThreshold(packet);
    }
  ...
  private void startHumidityGenerator() {
    if (humidity_step == 0) {
      return;
    }
    vertx.setPeriodic(humidity_generator_period, id -> {
      if (humidity_direction == Step.UP) {
        if (humidity >= humidity_max) {
          humidity_direction = Step.DOWN;
          this.humidity = (short) (humidity - humidity_step);
        } else {
          this.humidity = (short) (humidity + humidity_step);
        }
      } else {
        if (humidity <= humidity_min) {
          humidity_direction = Step.UP;
          this.humidity = (short) (humidity + humidity_step);
        } else {
          this.humidity = (short) (humidity - humidity_step);
        }
...
//fixme start_generator callback without sensor humidityReached
//fixme start_generator callback without sensor analogValueReached

    private void stopHumidityCallback() {
        vertx.cancelTimer(humidity_callback_id);
  }
...
//fixme stop_generator callback without sensor humidityReached
  ...
  private void startHumidityCallback() {
    ...
    humidity_callback_id = vertx.setPeriodic(humidityCallbackPeriod, id -> {
      if (humidity != humidity_last_value_called_back) {
        ...
        Set<Object> handlerids = vertx.sharedData().getLocalMap(Brickd.HANDLERIDMAP).keySet();
        if (handlerids != null) {
          logger.debug("humidity sending callback value");
          for (Object handlerid : handlerids) {
            vertx.eventBus().publish((String) handlerid, getHumidity4Callback());
          }
        } else {
          logger.info("no handlerids found in humidity callback");
        }
      } else {
        logger.debug("humidity value already called back");
      }
    });
  }

//fixme stop_generator callback without sensor humidity
...
//fixme getter callback without sensor humidityReached
//fixme getter callback without sensor analogValueReached

  private Buffer getHumidity4Callback() {
      byte options = (byte) 0;
      return getHumidityBuffer(CALLBACK_HUMIDITY, options);
  }
        //fixme getter callback without sensor humidity

  private Buffer setHumidityCallbackPeriod(Packet packet) {
    humidityCallbackPeriod =  Utils.unsignedInt(packet.getPayload());
    if (humidityCallbackPeriod == 0) {
      stopHumidityCallback();
    } else {
      startHumidityCallback();
    }
    if (packet.getResponseExpected()) {
      byte length = (byte) 8 + 0;
      byte functionId = FUNCTION_SET_HUMIDITY_CALLBACK_PERIOD;
      byte flags = (byte) 0;
  ...
  private Buffer getHumidityBuffer(byte functionId, byte options) {
    byte length = (byte) 8 + 2;
    byte flags = (byte) 0;
    Buffer header = Utils.createHeader(uidBytes, length, functionId, options, flags);
    Buffer buffer = Buffer.buffer();
    buffer.appendBuffer(header);
    buffer.appendBytes(Utils.getUInt16(humidity));

    return buffer;
  }
        
  private Buffer getHumidityBuffer(Packet packet) {
      byte options = packet.getOptions();
      return getHumidityBuffer(FUNCTION_GET_HUMIDITY, options);
  }

  private Buffer getHumidity(Packet packet) {
    logger.debug("function getHumidity");
    if (packet.getResponseExpected()) {
      return getHumidityBuffer(packet);
    }
    return null;
  }
   ...
   * 
   */
  private Buffer getHumidityCallbackThreshold(Packet packet) {
    logger.debug("function getHumidityCallbackThreshold");
    if (packet.getResponseExpected()) {
      byte length = (byte) 8 + 5;
      byte functionId = FUNCTION_GET_HUMIDITY_CALLBACK_THRESHOLD;
      byte flags = (byte) 0;
      Buffer header = Utils.createHeader(uidBytes, length, functionId, packet.getOptions(), flags);
      Buffer buffer = Buffer.buffer();
      buffer.appendBuffer(header);
      buffer.appendBuffer(this.humidityCallbackThreshold);
      return buffer;
    }
  ...
  private Buffer getHumidityCallbackThresholdDefault() {
      Buffer buffer = Buffer.buffer();
      buffer.appendBytes(Utils.getCharRandomValue(1));
      buffer.appendBytes(Utils.get2ByteURandomValue(1));
      buffer.appendBytes(Utils.get2ByteURandomValue(1));
   ...
   * 
   */
  private Buffer getHumidityCallbackPeriod(Packet packet) {
    logger.debug("function getHumidityCallbackPeriod");
    if (packet.getResponseExpected()) {
      byte length = (byte) 8 + 4;
      byte functionId = FUNCTION_GET_HUMIDITY_CALLBACK_PERIOD;
      byte flags = (byte) 0;
      Buffer header = Utils.createHeader(uidBytes, length, functionId, packet.getOptions(), flags);
      Buffer buffer = Buffer.buffer();
      buffer.appendBuffer(header);
    buffer.appendBytes(Utils.getUInt32(humidityCallbackPeriod));
...
//TODO response expected bei settern
   ...
   * 
   */
  private Buffer setHumidityCallbackThreshold(Packet packet) {
    logger.debug("function setHumidityCallbackThreshold");
    if (packet.getResponseExpected()) {
      byte length = (byte) 8 + 0;
      byte functionId = FUNCTION_SET_HUMIDITY_CALLBACK_THRESHOLD;
      byte flags = (byte) 0;
      Buffer header = Utils.createHeader(uidBytes, length, functionId, packet.getOptions(), flags);
      Buffer buffer = Buffer.buffer();
      buffer.appendBuffer(header);
//TODO response expected bei settern
    ...
    this.humidityCallbackThreshold = packet.getPayload();
    return null;
  }
